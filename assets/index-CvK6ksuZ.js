...
(Each letter is shifted +1 from the original.)`,hint:"Try shifting letters back by 1.",code:"CAS",explain:"DBT shifted back by 1 → CAS."}},{id:"node-4",type:"puzzle",name:"Steps Math",description:"Quick math check.",xp:50,puzzle:{type:"input",q:"If you walk 2 km and your step length is 0.8 m, about how many steps is that? (Round to whole number)",answers:["2500"],explain:"2000m / 0.8m ≈ 2500 steps."}},{id:"node-5",type:"puzzle",name:"CBS Motto",description:"Community memory.",xp:30,puzzle:{type:"code",q:"Type the CBS motto (3 words):",hint:"C _ _ _ _ _ _ _  B _ _ _ _ _  S _ _ _ _ _ _ _ _ _",code:"COMMUNITY BUILDS SOVEREIGNTY",explain:"CBS = Community Builds Sovereignty."}}],Cr="cbsgo_state_v6";function wo(t,n){try{const o=JSON.parse(t);return o&&typeof o=="object"?o:n}catch{return n}}function vo(){return{xp:0,completed:{},updatedAt:Date.now()}}function Ft(){const t=localStorage.getItem(Cr);return wo(t,vo())}function Er(t){t.updatedAt=Date.now(),localStorage.setItem(Cr,JSON.stringify(t))}function Hn(t){return 100+(Math.max(1,Number(t||1))-1)*40}function un(){return Number(Ft().xp||0)}function Ut(){const t=un();let n=1,o=t;for(;;){const l=Hn(n);if(o<l||(o-=l,n+=1,n>999))break}return n}function Mr(){const t=un();let n=1,o=t;for(;;){const l=Hn(n);if(o<l||(o-=l,n+=1,n>999))break}return o}function Lr(){return Hn(Ut())}function Dt(t){const n=Number(t||0);if(!Number.isFinite(n)||n<=0)return Ft();const o=Ut(),l=Ft();l.xp=Number(l.xp||0)+n,Er(l);const c=Ut();return window.dispatchEvent(new CustomEvent("cbsgo:xpChanged",{detail:{xp:l.xp,level:c}})),c>o&&(window.dispatchEvent(new CustomEvent("cbsgo:levelUp",{detail:{from:o,to:c,xp:l.xp}})),window.dispatchEvent(new CustomEvent("cbsgo:levelChanged",{detail:{level:c,xp:l.xp}}))),l}function Ar(t){const n=String(t||"");if(!n)return!1;const o=Ft();return!!(o.completed&&o.completed[n])}function Br(t){const n=String(t||"");if(!n)return;const o=Ft();o.completed||(o.completed={}),o.completed[n]=Date.now(),Er(o),window.dispatchEvent(new CustomEvent("cbsgo:nodeCompleted",{detail:{id:n}}))}const _o=Object.freeze(Object.defineProperty({__proto__:null,addXp:Dt,completeNode:Br,getLevel:Ut,getXp:un,getXpIntoLevel:Mr,getXpNeededThisLevel:Lr,isNodeCompleted:Ar},Symbol.toStringTag,{value:"Module"})),zr="cbsgoPuzzleModal";function So(t){return String(t).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function Nn(){const t=document.getElementById(zr);t&&t.remove()}function Rn(t){Nn();const n=6,o=6,l=["#f97373","#facc15","#4ade80","#60a5fa","#a855f7"],c=["🍬","💎","⭐","🍀","🔮"],f=180,u=18,p=l.length,b=.01;let x=[],h=null,y=0,_=u,T=!1,P=!1,E=null;const N=t?.name||"CBS GO Puzzle",q=document.createElement("div");q.id=zr,q.style.position="fixed",q.style.inset="0",q.style.zIndex="999999",q.style.display="flex",q.style.alignItems="center",q.style.justifyContent="center",q.style.padding="16px",q.style.background="rgba(0,0,0,.70)",q.style.backdropFilter="blur(12px)",q.style.fontFamily="system-ui, sans-serif",q.style.color="#fff",q.innerHTML=`
    <style>
  ...
  `,document.body.appendChild(q);const W=document.getElementById("cbsgoBoard"),be=document.getElementById("cbsgoScore"),ne=document.getElementById("cbsgoMoves"),se=document.getElementById("cbsgoStatus"),Se=document.getElementById("cbsgoPuzzleClose"),ke=document.getElementById("cbsgoPuzzleOk"),$e=document.getElementById("cbsgoConfettiLayer");function Ie(F){se&&(se.textContent=F||"")}function ot(){if(!$e)return;$e.style.display="block",$e.innerHTML="";const F=["#f97373","#facc15","#4ade80","#60a5fa","#a855f7","#ffffff"],Y=40;for(let K=0;K<Y;K++){const j=document.createElement("div"),te=6+Math.floor(Math.random()*6),me=Math.random()*100,Ee=Math.random()*.6,Me=1+Math.random()*.6,bt=Math.random()*360;j.style.position="absolute",j.style.top="-10%",j.style.left=`${me}%`,j.style.width=`${te}px`,j.style.height=`${te*2}px`,j.style.background=F[K%F.length],j.style.opacity="0.9",j.style.borderRadius="2px",j.style.transform=`rotate(${bt}deg)`,j.style.animation=`cbsgoConfettiFall ${Me}s ease-out ${Ee}s forwards`,$e.appendChild(j)}}function it(){return Math.floor(Math.random()*l.length)}function At(){x=[];for(let F=0;F<n;F++){const Y=[];for(let K=0;K<o;K++)Math.random()<b?Y.push(p):Y.push(it());x.push(Y)}}function at(F){return F===p}function Pe(){if(W){W.innerHTML="";for(let F=0;F<n;F++)for(let Y=0;Y<o;Y++){const K=x[F][Y],j=document.createElement("div");j.dataset.row=String(F),j.dataset.col=String(Y),j.style.borderRadius="12px",j.style.display="flex",j.style.alignItems="center",j.style.justifyContent="center",j.style.cursor=P?"default":"pointer",j.style.boxShadow="0 0 0 1px rgba(0,0,0,.18) inset",j.style.fontSize="20px",at(K)?(j.style.background="radial-gradient(circle at 30% 30%, #f97373, #a855f7)",j.textContent="💥"):(j.style.background=l[K]||"#444",j.textContent=c[K]||"⬛"),h&&h.row===F&&h.col===Y&&(j.style.outline="2px solid #fff",j.style.outlineOffset="2px"),j.addEventListener("click",()=>{je(F,Y)}),j.addEventListener("touchstart",te=>{if(P)return;const me=te.touches[0];E={row:F,col:Y,x:me.clientX,y:me.clientY}}),j.addEventListener("touchend",te=>{if(!E||P)return;const me=te.changedTouches[0],Ee=me.clientX-E.x,Me=me.clientY-E.y;if(Math.sqrt(Ee*Ee+Me*Me)<18){je(F,Y),E=null;return}let De=E.row,Je=E.col;Math.abs(Ee)>Math.abs(Me)?Ee>0?Je+=1:Je-=1:Me>0?De+=1:De-=1,De>=0&&De<n&&Je>=0&&Je<o&&Oe(E.row,E.col,De,Je),E=null,te.preventDefault()}),W.appendChild(j)}}}function xt(F,Y){if(!F||!Y)return!1;const K=Math.abs(F.row-Y.row),j=Math.abs(F.col-Y.col);return K+j===1}function Ue(F,Y){const K=x[F.row][F.col];x[F.row][F.col]=x[Y.row][Y.col],x[Y.row][Y.col]=K}function yt(){const F=new Set;for(let Y=0;Y<n;Y++){let K=x[Y][0],j=0;for(let te=1;te<=o;te++){const me=te<o?x[Y][te]:null;if(me===K)continue;const Ee=te-j;if(K!=null&&Ee>=3)for(let Me=j;Me<te;Me++)F.add(`${Y},${Me}`);K=me,j=te}}for(let Y=0;Y<o;Y++){let K=x[0][Y],j=0;for(let te=1;te<=n;te++){const me=te<n?x[te][Y]:null;if(me===K)continue;const Ee=te-j;if(K!=null&&Ee>=3)for(let Me=j;Me<te;Me++)F.add(`${Me},${Y}`);K=me,j=te}}return F}function qe(F){if(!F||!F.size)return 0;const Y=F.size;y+=Y*4,be&&(be.textContent=String(y)),!P&&y>=f&&gt(!0);for(const K of F){const[j,te]=K.split(","),me=Number(j),Ee=Number(te);x[me][Ee]=null}for(let K=0;K<o;K++){let j=n-1;for(let te=n-1;te>=0;te--)x[te][K]!=null&&(x[j][K]=x[te][K],j--);for(let te=j;te>=0;te--)Math.random()<b?x[te][K]=p:x[te][K]=it()}return Y}function Qe(F,Y){const K=new Set;for(let j=0;j<o;j++)K.add(`${F},${j}`);for(let j=0;j<n;j++)K.add(`${j},${Y}`);qe(K),Pe(),P||setTimeout(()=>Bt(!1),120)}function Bt(F=!1){if(P)return;T=!0;const Y=()=>{if(P){T=!0;return}const K=yt();if(!K.size){T=!1,Pe(),F&&!P&&(_<=0?Ke():Ie("Nice! Keep matching."));return}qe(K),Pe(),setTimeout(Y,120)};Y()}function gt(F){if(!P)if(P=!0,T=!0,F){Ie("Great job! Puzzle completed 🎉");try{t?.id&&Br(t.id),Dt(10)}catch{}ot(),setTimeout(()=>{Nn()},1600)}else Ie("Out of moves. Try again next time 🙂")}function Ke(){y>=f?gt(!0):_<=0&&gt(!1)}function Oe(F,Y,K,j){if(T||P)return;if(_<=0){Ke();return}const te={row:F,col:Y},me={row:K,col:j};if(!xt(te,me))return;const Ee=x[F][Y],Me=x[K][j],bt=at(Ee)||at(Me);if(Ue(te,me),h=null,_--,ne&&(ne.textContent=String(_)),bt){Pe();const De=at(x[F][Y])?{row:F,col:Y}:{row:K,col:j};Qe(De.row,De.col),Ke();return}if(!yt().size){Ue(te,me),Pe(),Ie("No match… try another swap."),Ke();return}Ie(""),Pe(),Bt(!0)}function je(F,Y){if(T||P)return;if(_<=0){Ke();return}const K={row:F,col:Y};if(!h){h=K,Pe();return}if(h.row===F&&h.col===Y){h=null,Pe();return}if(!xt(h,K)){h=K,Pe();return}Oe(h.row,h.col,K.row,K.col)}function de(){Nn()}Se&&(Se.onclick=de),ke&&(ke.onclick=()=>{de()}),At(),Pe(),Ie("Tap or swipe two neighboring tiles to swap them.")}const Nr="cbsgo_inventory_v2";function ko(t,n){try{const o=JSON.parse(t);return o&&typeof o=="object"?o:n}catch{return n}}function Co(){return{tickets:0,cbs:0,cards:{}}}function dt(){const t=localStorage.getItem(Nr),n=ko(t,Co());return typeof n.tickets!="number"&&(n.tickets=0),typeof n.cbs!="number"&&(n.cbs=0),(!n.cards||typeof n.cards!="object")&&(n.cards={}),n}function Xn(t){const n={tickets:Number(t.tickets||0),cbs:Number(t.cbs||0),cards:t.cards&&typeof t.cards=="object"?t.cards:{}};localStorage.setItem(Nr,JSON.stringify(n))}function Jt(){return Number(dt().tickets||0)}function en(){return Number(dt().cbs||0)}function Lt(t=1){const n=Number(t||0);if(!Number.isFinite(n)||n===0)return dt();const o=dt();let c=Number(o.tickets||0)+n;return c<0&&(c=0),o.tickets=c,Xn(o),window.dispatchEvent(new CustomEvent("cbsgo:inventoryChanged",{detail:{...o}})),o}function xn(t=1){const n=Number(t||0);if(!Number.isFinite(n)||n===0)return dt();const o=dt();let c=Number(o.cbs||0)+n;return c<0&&(c=0),o.cbs=c,Xn(o),window.dispatchEvent(new CustomEvent("cbsgo:inventoryChanged",{detail:{...o}})),o}const Ir="cbsgo_steps_v6",Eo="cbsgo_steps_v5",Mo="cbsgo_gps_autostart_v2",Tr="cbsgo_daily_puzzle_v1",Lo=.75,Ct=5e3,sn=7,Fn=100,Ao=1e3,Bo=.5,zo=2e3,No=4.5,In=1500,Tn=200,Io=.25,To=.05,$o=.3;let tn=null,nn=!1,wt={msg:"init"};function Un(t,n){try{const o=JSON.parse(t);return o&&typeof o=="object"?o:n}catch{return n}}const $r="cbsgo_cards_v1",Po=[{id:"walk_sun_1",name:"Sunny Walk",emoji:"☀️",rarity:"common"},{id:"cbs_heart_1",name:"CBS Heart",emoji:"❤️",rarity:"rare"}];function Oo(t){if(!t)return null;const n=String(t);return n==="sunny_walker"?"walk_sun_1":n==="cbs_heart"?"cbs_heart_1":n}function jo(t){return Po.find(n=>n.id===t)||{id:t,name:t,emoji:"🃏",rarity:"common"}}function Ro(){try{const t=localStorage.getItem($r),n=Un(t,{});if(n&&typeof n.counts=="object"&&n.counts!==null)return{counts:{...n.counts}};if(n&&typeof n=="object"&&!Array.isArray(n)){const o={};for(const[l,c]of Object.entries(n))if(c&&typeof c=="object"&&"count"in c){const f=Number(c.count);Number.isFinite(f)&&f>0&&(o[l]=f)}if(Object.keys(o).length>0)return{counts:o}}return{counts:{}}}catch{return{counts:{}}}}function Fo(t){try{const n=t&&t.counts&&typeof t.counts=="object"?t.counts:{},o={};for(const[c,f]of Object.entries(n)){const u=Number(f||0);Number.isFinite(u)&&u>0&&(o[c]=u)}const l={counts:o};localStorage.setItem($r,JSON.stringify(l))}catch{}}function Uo(t,n=1){const o=Oo(t);if(!o)return null;const l=Number(n||0);if(!Number.isFinite(l)||l<=0)return null;const f={...Ro().counts||{}},p=Number(f[o]||0)+l;f[o]=p,Fo({counts:f});const b=jo(o);try{window.dispatchEvent(new CustomEvent("cbsgo:cardsChanged",{detail:{counts:f}})),window.dispatchEvent(new CustomEvent("cbsgo:cardFound",{detail:{cardId:o,count:p,card:b}})),window.dispatchEvent(new CustomEvent("cbsgo:bagChanged",{detail:{reason:"cardDrop",card:b}}))}catch{}return{cardId:o,count:p,card:b}}function rt(){const t=new Date,n=t.getFullYear(),o=String(t.getMonth()+1).padStart(2,"0"),l=String(t.getDate()).padStart(2,"0");return`${n}-${o}-${l}`}function Go(t){if(!t||typeof t!="string")return null;const n=t.split("-").map(Number);if(n.length!==3)return null;const[o,l,c]=n,f=new Date(o,l-1,c);return Number.isNaN(f.getTime())?null:f}function Wo(t){const n=t.getFullYear(),o=String(t.getMonth()+1).padStart(2,"0"),l=String(t.getDate()).padStart(2,"0");return`${n}-${o}-${l}`}function Pr(t,n){const o=Go(t);if(!o)return[];const l=[];for(let c=n-1;c>=0;c--){const f=new Date(o.getTime());f.setDate(f.getDate()-c),l.push(Wo(f))}return l}function ln(){return{steps:0,meters:0,totalMeters:0,lastPos:null,boostUntil:0,boostLastStep:0,chestMeters:0,xpKmAwarded:0,ticketChunksAwarded:0,dayKey:rt(),daySteps:0,dayMeters:0,dailyGoalSteps:Ct,dailyGoalReached:!1,streak:{},lastStreakRewardDate:null,dailyVersion:1,updatedAt:Date.now()}}function Do(t){const n=rt();return!t||typeof t!="object"?ln():(typeof t.steps!="number"&&(t.steps=0),typeof t.meters!="number"&&(t.meters=0),typeof t.chestMeters!="number"&&(t.chestMeters=0),typeof t.xpKmAwarded!="number"&&(t.xpKmAwarded=0),typeof t.ticketChunksAwarded!="number"&&(t.ticketChunksAwarded=0),typeof t.totalMeters!="number"&&(t.totalMeters=Number(t.meters||0)),typeof t.dailyVersion!="number"||t.dailyVersion<1?(t.dayKey||(t.dayKey=n),typeof t.daySteps!="number"&&(t.daySteps=t.steps||0),typeof t.dayMeters!="number"&&(t.dayMeters=t.meters||0),(typeof t.dailyGoalSteps!="number"||t.dailyGoalSteps<=0)&&(t.dailyGoalSteps=Ct),typeof t.dailyGoalReached!="boolean"&&(t.dailyGoalReached=!1),(!t.streak||typeof t.streak!="object")&&(t.streak={}),typeof t.lastStreakRewardDate!="string"&&(t.lastStreakRewardDate=null),t.dailyVersion=1):(t.dayKey||(t.dayKey=n),typeof t.daySteps!="number"&&(t.daySteps=0),typeof t.dayMeters!="number"&&(t.dayMeters=0),(typeof t.dailyGoalSteps!="number"||t.dailyGoalSteps<=0)&&(t.dailyGoalSteps=Ct),typeof t.dailyGoalReached!="boolean"&&(t.dailyGoalReached=!1),(!t.streak||typeof t.streak!="object")&&(t.streak={}),typeof t.lastStreakRewardDate!="string"&&(t.lastStreakRewardDate=null)),t)}function yn(t){t.updatedAt=Date.now(),localStorage.setItem(Ir,JSON.stringify(t))}function Yo(t,n){if(!n)return;const o=Pr(n,sn);!o.length||!o.every(c=>!!t.streak[c])||t.lastStreakRewardDate!==n&&(xn(Fn),Yt(),t.lastStreakRewardDate=n,window.dispatchEvent(new CustomEvent("cbsgo:streakReward",{detail:{days:sn,rewardCbs:Fn,lastDayKey:n}})))}function pr(t){t=Do(t||ln());const n=rt();if(t.dayKey!==n){const o=t.dayKey;o&&(t.streak||(t.streak={}),t.streak[o]=!!t.dailyGoalReached,Yo(t,o)),t.dayKey=n,t.daySteps=0,t.dayMeters=0,t.dailyGoalReached=!1,yn(t)}return t}function pt(){let t=localStorage.getItem(Ir);if(!t){const o=localStorage.getItem(Eo);if(o){const l=Un(o,ln()),c=pr(l);return yn(c),c}}const n=Un(t,ln());return pr(n)}function rn(){window.dispatchEvent(new CustomEvent("cbsgo:stepsChanged",{detail:{steps:qo()}}))}function Vn(){window.dispatchEvent(new CustomEvent("cbsgo:xpChanged",{}))}function Yt(){window.dispatchEvent(new CustomEvent("cbsgo:inventoryChanged",{}))}function Zn(t,n,o,l){const c=Number(t||0),f=Number(n||0),u=0;if(!(!c&&!f&&!u))try{window.dispatchEvent(new CustomEvent("cbsgo:stepReward",{detail:{xp:c,tickets:f,cbs:u,reason:l||"distance"}}))}catch{}}function qo(){const t=pt();return Number(t.daySteps!=null?t.daySteps:t.steps||0)}function Ko(){const t=pt(),n=t.dayMeters!=null?t.dayMeters:t.meters||0;return Number(n||0)}function Ho(){return Ko()/1e3}function Xo(){const t=pt(),n=Number(t.daySteps!=null?t.daySteps:t.steps||0),o=Number(t.dailyGoalSteps||Ct),l=!!t.dailyGoalReached,c=t.dayKey||rt(),f=t.streak||{},p=Pr(c,sn).map(b=>{let x=!1;return b===c?x=l:x=!!f[b],{dateKey:b,reached:x}});return{stepsToday:n,goalSteps:o,goalReached:l,streak:p,todayKey:c,streakLength:sn,rewardPerStreak:Fn}}function ur(){return!!nn}function Vo(){try{return localStorage.getItem(Tr)===rt()}catch{return!1}}function Zo(){try{localStorage.setItem(Tr,rt())}catch{}}function Qo(t,n){return Vo()?!1:(window.dispatchEvent(new CustomEvent("cbsgo:dailyPuzzle",{detail:{lat:t,lng:n,date:rt()}})),Zo(),!0)}function xr(){const t=pt(),n=Number(t.boostUntil||0);return Math.max(0,n-Date.now())}function Jo(t){if(!Math.max(0,Number(t.boostUntil||0)-Date.now()))return;const o=Number(t.boostLastStep||0),l=Number(t.steps||0);if(!Number.isFinite(o)){t.boostLastStep=l;return}const c=l-o;if(!Number.isFinite(c)||c<In)return;const f=Math.floor(c/In);f<=0||(Lt(f),Yt(),Zn(0,f,0,"boost"),t.boostLastStep=o+f*In)}function ei(t){let n=Number(t.chestMeters||0);if(Number.isFinite(n)||(n=0),n<Tn){t.chestMeters=n;return}let o=0;for(;n>=Tn&&o<5;)if(n-=Tn,o+=1,Math.random()<Io){const l=Math.random()<To,c=l?10:3,f=l?2:1;Dt(c),Vn(),Lt(f),Yt();const u=l&&Math.random()<$o;Zn(c,f,0,l?"treasure-rare":"treasure"),window.dispatchEvent(new CustomEvent("cbsgo:treasureFound",{detail:{xp:c,tickets:f,rare:l,hasCBSFlag:u}}));break}t.chestMeters=n}function ti(t,n){const l=x=>x*Math.PI/180,c=l(n.lat-t.lat),f=l(n.lng-t.lng),u=l(t.lat),p=l(n.lat),b=Math.sin(c/2)**2+Math.cos(u)*Math.cos(p)*Math.sin(f/2)**2;return 2*6371e3*Math.asin(Math.sqrt(b))}function ni(t){const n=Number((t.totalMeters!=null?t.totalMeters:t.meters)||0);if(!Number.isFinite(n)||n<=0)return;let o=0,l=0;const c=Math.floor(n/1e3),f=Number(t.xpKmAwarded||0);if(c>f){const x=c-f;x>0&&(Dt(x),Vn(),t.xpKmAwarded=c,o+=x)}const p=Math.floor(n/2500),b=Number(t.ticketChunksAwarded||0);if(p>b){const x=p-b;x>0&&(Lt(x),Yt(),t.ticketChunksAwarded=p,l+=x)}(o>0||l>0)&&Zn(o,l,0,"distance")}function ri(t){const n=Number(t||0);if(!Number.isFinite(n)||n<=0)return pt();const o=pt();o.totalMeters=Number(o.totalMeters||0)+n,o.meters=Number(o.meters||0)+n,o.dayMeters=Number(o.dayMeters||0)+n,o.chestMeters=Number(o.chestMeters||0)+n;const l=Number(o.steps||0),c=Math.floor((o.meters||0)/Lo);if(c>l){const f=c-l;o.steps=c,o.daySteps=Number(o.daySteps||0)+f}return!o.dailyGoalReached&&o.daySteps>=(o.dailyGoalSteps||Ct)&&(o.dailyGoalReached=!0,window.dispatchEvent(new CustomEvent("cbsgo:dailyGoalReached",{detail:{dayKey:o.dayKey||rt(),steps:o.daySteps,goal:o.dailyGoalSteps||Ct}}))),ni(o),Jo(o),ei(o),yn(o),rn(),o}function oi(){tn!=null&&navigator.geolocation&&navigator.geolocation.clearWatch(tn),tn=null}async function yr(t={}){const n=!!t.silent;if(!navigator.geolocation)return wt={err:"GPS not supported",t:Date.now()},{ok:!1,reason:"GPS not supported"};try{localStorage.setItem(Mo,"1")}catch{}oi(),nn=!0,wt={msg:"requesting",t:Date.now()};try{return tn=navigator.geolocation.watchPosition(o=>{const l=o.coords.latitude,c=o.coords.longitude,f=o.coords.accuracy||999,u=Date.now(),p=pt(),b=p.lastPos;p.lastPos={lat:l,lng:c,t:u},yn(p);const x=Number.isFinite(o.coords.heading)?o.coords.heading:null,h=Number.isFinite(o.coords.speed)?o.coords.speed:null;if(window.dispatchEvent(new CustomEvent("cbsgo:playerPos",{detail:{lat:l,lng:c,acc:f,heading:x,speed:h,t:u}})),f>Ao){wt={lat:l,lng:c,acc:f,t:u,reason:"accuracy",boostMs:xr()},rn();return}Qo(l,c);let y=0,_=0,T=0,P=0,E="no-last";b&&typeof b.lat=="number"&&typeof b.lng=="number"&&typeof b.t=="number"&&(y=ti({lat:b.lat,lng:b.lng},{lat:l,lng:c}),_=Math.max(1,(u-b.t)/1e3),T=y/_,y<Bo?E="jitter":y>zo?E="teleport":T>No?E="too-fast":(ri(y),P=y,E="ok")),wt={lat:l,lng:c,acc:f,t:u,dist:Math.round(y),dt:Math.round(_),speed:Number.isFinite(T)?Number(T.toFixed(2)):0,added:Math.round(P),reason:E,boostMs:xr()},rn()},o=>{nn=!1,wt={err:o?.message||"GPS blocked",t:Date.now()},rn()},{enableHighAccuracy:!0,maximumAge:1e3,timeout:12e3}),{ok:!0}}catch(o){return nn=!1,wt={err:String(o?.message||o),t:Date.now()},{ok:!1,reason:"Failed to start GPS"}}}function ii(){if(window.__cbsgo_try_autostart)return;window.__cbsgo_try_autostart=!0,(async()=>ur()||await yr({silent:!0}))();const n=async()=>{ur()||await yr({silent:!0}),window.removeEventListener("pointerdown",n),window.removeEventListener("touchstart",n),window.removeEventListener("click",n)};window.addEventListener("pointerdown",n,{once:!0}),window.addEventListener("touchstart",n,{once:!0}),window.addEventListener("click",n,{once:!0})}window.__cbsgo_loot_reward_listener_v1||(window.__cbsgo_loot_reward_listener_v1=!0,window.addEventListener("cbsgo:lootReward",t=>{const n=t?.detail||{},o=Number(n.xp||0),l=Number(n.tickets||0),c=Number(n.cbs||0);o>0&&(Dt(o),Vn()),(l>0||c>0)&&(l>0&&Lt(l),c>0&&xn(c),Yt());const f=n.cardId||n.card_id;if(f)try{const u=Number(n.cardCount||n.count||1)>0?Number(n.cardCount||n.count||1):1;Uo(f,u)}catch(u){console.warn("CBS GO: grantCard from lootReward failed",u)}}));function Or(){const t=un(),n=Ut(),o=Mr(),l=Lr(),c=Ho(),f=l>0?Math.min(100,Math.round(o/l*100)):0;return`
  ...
  `}function jr(){const{stepsToday:t,goalSteps:n,goalReached:o,streak:l,streakLength:c,rewardPerStreak:f}=Xo(),u=n>0?Math.min(100,Math.round(t/n*100)):0,p=(l||[]).map(x=>x.reached?"★":"☆").join(" ");return`
  ...
  `}function Rr(){try{return new URLSearchParams(window.location.search).get("dev")==="1"}catch{return!1}}function ai(){try{const t=[];for(let n=0;n<localStorage.length;n++){const o=localStorage.key(n);o&&o.startsWith("cbsgo_")&&t.push(o)}t.forEach(n=>localStorage.removeItem(n))}catch{}try{const t=[];for(let n=0;n<sessionStorage.length;n++){const o=sessionStorage.key(n);o&&o.startsWith("cbsgo_")&&t.push(o)}t.forEach(n=>sessionStorage.removeItem(n))}catch{}window.location.reload()}const Fr="cbsgo_player_name_v2",Qn="cbsgo_player_avatar_v2";function ut(){try{return localStorage.getItem(Fr)||"Sovereign"}catch{return"Sovereign"}}function Ur(t){const n=String(t||"").trim().slice(0,24)||"Sovereign";try{localStorage.setItem(Fr,n)}catch{}return n}function gn(){try{return localStorage.getItem(Qn)||""}catch{return""}}function si(t){const n=String(t||"");try{localStorage.setItem(Qn,n)}catch{}return n}function li(){try{localStorage.removeItem(Qn)}catch{}}let X=null,et=null,tt=null,$t=null,Ot=null,We=null,Te=null,vt=0,ct=!1,Ze=!0,Ge=null;const Xe=new Map;let Ve=!0,jt={temp:null,iconEmoji:"⛅",condition:"clear",isNight:!1,lastUpdated:0};const ci="48a387bba00043ac4ba5823371abc9d2",Gt=80,fi=6,di=80,pi=220,ui=6e4,xi=5*6e4,yi=300,gi=.35,$n=["walk_sun_1","walk_rain_1","walk_city_1","cbs_heart_1"],bi=350,hi=.35,mi=120;let cn=0,_t=0,on=null,Gn=!1,kt=[];function ft(t){return document.getElementById(t)}function St(t){const n=ft("cbsgoMapHost");if(!n)return;let o=ft("cbsgoMapMsg");o||(o=document.createElement("div"),o.id="cbsgoMapMsg",o.style.position="absolute",o.style.left="12px",o.style.right="12px",o.style.bottom="16px",o.style.zIndex="9999",o.style.padding="10px 12px",o.style.borderRadius="14px",o.style.border="1px solid rgba(255,255,255,.14)",o.style.background="rgba(0,0,0,.40)",o.style.color="#fff",o.style.fontFamily="system-ui, sans-serif",o.style.fontSize="13px",o.style.backdropFilter="blur(10px)",n.appendChild(o)),o.textContent=t||""}function wi(){const t=String(ut()||"").trim();return t?t[0].toUpperCase():"🙂"}function Wn(t){return String(t||"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function Et(t,n){const l=x=>x*Math.PI/180,c=l(n.lat-t.lat),f=l(n.lng-t.lng),u=l(t.lat),p=l(n.lat),b=Math.sin(c/2)**2+Math.cos(u)*Math.cos(p)*Math.sin(f/2)**2;return 2*6371e3*Math.asin(Math.sqrt(b))}function Gr(t,n,o){const l=n+Math.random()*(o-n),c=Math.random()*2*Math.PI,f=l*Math.cos(c)/111111,u=l*Math.sin(c)/(111111*Math.cos(t.lat*Math.PI/180));return{lat:t.lat+f,lng:t.lng+u}}function vi(t,n){const o=x=>x*Math.PI/180,l=o(t.lat),c=o(n.lat),f=o(n.lng-t.lng),u=Math.sin(f)*Math.cos(c),p=Math.cos(l)*Math.sin(c)-Math.sin(l)*Math.cos(c)*Math.cos(f);let b=Math.atan2(u,p);return b=b*180/Math.PI,b=(b+360)%360,b}function _i(t,n,o){const c=n/6371e3,f=o*Math.PI/180,u=t[0]*Math.PI/180,p=t[1]*Math.PI/180,b=Math.sin(u),x=Math.cos(u),h=Math.sin(c),y=Math.cos(c),_=Math.asin(b*y+x*h*Math.cos(f)),T=p+Math.atan2(Math.sin(f)*h*x,y-b*Math.sin(_));return[_*180/Math.PI,T*180/Math.PI]}function Si(){if(document.getElementById("cbsgoWeatherFxStyles"))return;const t=document.createElement("style");t.id="cbsgoWeatherFxStyles",t.textContent=`
  ...
  `,document.head.appendChild(t)}function Wr(){const{temp:t,iconEmoji:n}=jt;return t==null?"⛅ …°":`${n} ${Math.round(t)}°`}function Dr(){const t=document.getElementById("cbsgoWeatherFx");if(!t)return;Si();const{condition:n,isNight:o}=jt;t.style.background=o?"radial-gradient(ellipse at top, rgba(0,0,0,0.45), rgba(0,0,0,0.85))":"transparent";let l="";if(n==="rain"||n==="storm"){const f=[];for(let u=0;u<96;u++){const p=Math.random()*100,b=Math.random()*16-8,x=Math.random()*2.5,h=2+Math.random()*1.5;f.push(`
      ...
      `)}l=f.join("")}else if(n==="snow"){const f=[];for(let u=0;u<80;u++){const p=Math.random()*100,b=Math.random()*20-10,x=Math.random()*4,h=6+Math.random()*4;f.push(`
        <div
          ...
          "
        ></div>
      `)}l=f.join("")}else l="";t.innerHTML=l}async function ki(t,n){const o=Date.now();if(!(jt.lastUpdated&&o-jt.lastUpdated<300*1e3))try{const l=`https://api.openweathermap.org/data/2.5/weather?lat=${t}&lon=${n}&appid=${ci}&units=metric`,c=await fetch(l);if(!c.ok)throw new Error("HTTP "+c.status);const f=await c.json(),u=f?.main?.temp,p=f?.weather?.[0]?.icon||"01d",b=String(f?.weather?.[0]?.main||"").toLowerCase();let x=p.endsWith("n"),h="⛅",y="clear";p.startsWith("01")||p.startsWith("02")?y="clear":p.startsWith("03")||p.startsWith("04")?(h="☁️",y="clouds"):p.startsWith("09")||p.startsWith("10")?(h="🌧️",y="rain"):p.startsWith("11")?(h="⛈️",y="storm"):p.startsWith("13")?(h="❄️",y="snow"):p.startsWith("50")&&(h="🌫️",y="mist"),b.includes("rain")&&(y="rain"),b.includes("snow")&&(y="snow"),b.includes("thunder")&&(y="storm");try{const T=Number(f?.dt||0),P=Number(f?.timezone||0);if(T&&Number.isFinite(P)){const N=((T+P)/3600%24+24)%24;x=N<7||N>=19}}catch{}y==="clear"?h=x?"🌙":"☀️":y==="clouds"?h="☁️":y==="rain"?h="🌧️":y==="storm"?h="⛈️":y==="snow"?h="❄️":y==="mist"&&(h="🌫️"),jt={temp:u,iconEmoji:h,condition:y,isNight:x,lastUpdated:o};const _=document.getElementById("cbsgoWeatherLabel");_&&(_.textContent=Wr()),Dr()}catch(l){console.warn("Weather fetch failed",l)}}function Ci(t){const n=gn();if(n){const c=`
      <div style="
  ...
  `;return t.divIcon({html:o,className:"",iconSize:[28,28],iconAnchor:[14,14]})}function Ei(t,n,o,l){if(!l&&o){const p=`
    ...
    ">${Wn(c)}</div>
  `;return t.divIcon({html:f,className:"",iconSize:[30,30],iconAnchor:[15,15]})}function Mi(t){return t.divIcon({html:`
    <div style="
      position:relative;
...
${ne}`);const se={kind:"mystery",xp:P||0,tickets:E||0,cbs:N||0,cardId:q||null,cardCount:W||0};try{window.dispatchEvent(new CustomEvent("cbsgo:lootReward",{detail:se}))}catch{}}),b.addTo(We),cn=n}function Ii(t){if(!X||!We||!t)return;const n=Date.now();let o=0;kt=kt.filter(l=>{if(!l||!l.marker||!We.hasLayer(l.marker))return!1;if(n-(l.createdAt||0)>xi)return We.removeLayer(l.marker),o+=1,!1;const f=Et({lat:t.lat,lng:t.lng},{lat:l.lat,lng:l.lng});return Number.isFinite(f)&&f>yi?(We.removeLayer(l.marker),o+=1,!1):!0}),o>0&&We.getLayers().length===0&&(cn=0)}function Ti(t){if(!X||!Ot||!t||on)return;const n=window.L;if(!n)return;if(Gn){if(_t<bi||Math.random()>hi)return;_t=0}else{if(_t<mi)return;_t=0,Gn=!0}const o=Gr(t,60,140),l=Li(n),c=n.marker([o.lat,o.lng],{icon:l,pane:"cbsgo-puzzle-pane"});c.on("click",()=>{if(!Te){alert("GPS not ready yet. Wait until your player marker appears.");return}const f={lat:Te[0],lng:Te[1]},u={lat:o.lat,lng:o.lng},p=Et(f,u);if(p>Gt){alert(`Too far to start this puzzle.

Distance: ${Math.round(p)}m
Needed: ≤ ${Gt}m`);return}Ot.removeLayer(c),on=null,Rn({id:`puzzle-${Date.now()}`,name:"CBS GO Puzzle"})}),c.addTo(Ot),on=c}function $i(t){const n=window.L;if(!n||!X||!t)return;const o=Gt;$t?($t.setLatLng(t),$t.setRadius(o)):$t=n.circle(t,{radius:o,color:"#38bdf8",weight:1,opacity:.9,fillColor:"#38bdf8",fillOpacity:.12,dashArray:"4 6"}).addTo(X)}function Pi(t){const n=window.L;if(!n||!X)return;const o=Ci(n);if(et?(et.setIcon(o),et.setLatLng(t)):(et=n.marker(t,{icon:o,pane:"cbsgo-player-pane"}).addTo(X),X.setView(t,19)),tt?(tt.setIcon(gr(n,vt)),tt.setLatLng(t)):tt=n.marker(t,{icon:gr(n,vt),interactive:!1,pane:"cbsgo-player-pane"}).addTo(X),et&&et.bringToFront&&et.bringToFront(),tt&&tt.bringToFront&&tt.bringToFront(),$i(t),Ze&&!ct&&X)try{const l=X.getZoom()||19;let c=t;Number.isFinite(vt)&&(c=_i(t,40,vt));const f=X.getCenter(),u=Et({lat:f.lat,lng:f.lng},{lat:c[0],lng:c[1]});(!Number.isFinite(u)||u>20)&&X.setView(c,l)}catch{}}function Yr(){const t=window.L;return!t||!X?null:(Ge?(Ve&&!X.hasLayer(Ge)&&Ge.addTo(X),!Ve&&X.hasLayer(Ge)&&X.removeLayer(Ge)):(Ge=t.layerGroup(),Ve&&Ge.addTo(X)),Ge)}function Oi(t){if(!Array.isArray(t)||!X)return[];const n=X.getZoom()||3;let o;n>=15?o=100:n>=10?o=50:n>=6?o=25:o=10;const l=new Map;t.forEach(f=>{if(!f||f.isMe||typeof f.lat!="number"||typeof f.lng!="number")return;const u=Math.round(f.lat*o)/o,p=Math.round(f.lng*o)/o,b=`${u}_${p}`;l.has(b)||l.set(b,[]),l.get(b).push(f)});const c=[];for(const[f,u]of l.entries())if(u.length)if(u.length===1){const p=u[0];c.push({id:p.wallet_pk||f,lat:p.lat,lng:p.lng,count:1,nickname:p.nickname||"Anon",avatar:p.avatar||"",isCluster:!1})}else{let p=0,b=0;u.forEach(y=>{p+=y.lat,b+=y.lng});const x=p/u.length,h=b/u.length;c.push({id:`cluster_${f}`,lat:x,lng:h,count:u.length,nickname:`${u.length} players`,avatar:"",isCluster:!0})}return c}function ji(t){const n=window.L;if(!n||!X)return;const o=Yr();if(!o)return;if(!Ve){for(const[f,u]of Xe.entries())o.removeLayer(u),Xe.delete(f);return}const l=Oi(t),c=new Set;l.forEach(f=>{if(!f||typeof f.lat!="number"||typeof f.lng!="number")return;const u=f.id||`${f.lat},${f.lng}`;c.add(u);const p=[f.lat,f.lng];let b=Xe.get(u);if(b)b.setLatLng(p);else{const x=f.isCluster&&f.count>1?String(f.count):f.nickname||"Anon",h=Ei(n,x,f.avatar,f.isCluster);b=n.marker(p,{icon:h,pane:"cbsgo-others-pane"});const y=f.isCluster&&f.count>1?`${f.count} CBS-GO explorers nearby`:`${f.nickname||"CBS-GO explorer"}`;b.bindPopup(y),b.addTo(o),Xe.set(u,b)}});for(const[f,u]of Xe.entries())c.has(f)||(o.removeLayer(u),Xe.delete(f))}function Ri(){return`
  ...
  `:`
    ...
    `}function Hi(){const t=qi(),{collected:n,total:o}=t,l=`${n}/${o} cards collected`,c=o>0?Math.round(n/o*100):0;return`
    <div style="
      display:flex;
      flex-direction:column;
      gap:10px;
  ...
  `}function hr(t=!0){return`
    ...
    font-weight:900;
    cursor:pointer;
  `}function xa(){const t=!Hr();let n="";try{const h=ut();t?h&&h!=="Sovereign"?n=h:n="":n=h||""}catch{n=""}const o=t?`
    ...
    `,l=pa(ua(t?"Welcome to CBS-GO":"Unlock Wallet",o)),c=l.querySelector("#cbsgoLoginMsg"),f=h=>{c&&(c.textContent=h||"")},u=l.querySelector("#cbsgoPin"),p=l.querySelector("#cbsgoPin2"),b=l.querySelector("#cbsgoNick"),x=()=>{Zr(),window.dispatchEvent(new CustomEvent("cbsgo:loginDone",{detail:{}}))};if(t){const h=l.querySelector("#cbsgoCreateBtn");h&&(h.onclick=async()=>{try{const y=String(b?.value||"").trim(),_=String(u?.value||"").trim(),T=String(p?.value||"").trim();if(y.length<2)return f("⛔ Nickname too short.");if(_.length<4)return f("⛔ PIN must be at least 4 digits.");if(_!==T)return f("⛔ PINs do not match.");f("Creating wallet…"),Ur(y),await ca(_),f("✅ Wallet created. Starting…"),x()}catch(y){f(`⛔ ${String(y?.message||y)}`)}})}else{const h=l.querySelector("#cbsgoUnlockBtn");h&&(h.onclick=async()=>{try{const y=String(u?.value||"").trim();if(y.length<4)return f("⛔ PIN must be at least 4 digits.");f("Unlocking…"),await fa(y),f("✅ Unlocked."),x()}catch{f("⛔ Wrong PIN (or wallet data missing).")}})}}const ya="https://cxfedvowjgkqrakkkjpi.supabase.co",ga="sb_publishable_Eed_27P11ry75z-QL7gGCg_6VQ4WTqT",Fe=po(ya,ga);function ba(){const t=Ye();if(!t)return null;const n=ut(),o=gn();return{wallet_pk:t,nickname:n,avatar:o}}async function an(t={}){try{const n=ba();if(!n){console.warn("CBS GO: no local wallet, skip profile sync");return}const o={...n,...t,last_seen:new Date().toISOString()},{error:l}=await Fe.from("players").upsert(o,{onConflict:"wallet_pk"});l&&console.warn("CBS GO: failed to sync player profile",l)}catch(n){console.warn("CBS GO: syncPlayerProfile crashed",n)}}const ha=15e3,ma=1e4,wa=300*1e3;let Pt=null,mr=0,wr=0;function va(t){const n=t?.detail||{};typeof n.lat!="number"||typeof n.lng!="number"||(Pt={lat:n.lat,lng:n.lng,heading:typeof n.heading=="number"?n.heading:null,acc:typeof n.acc=="number"?n.acc:null,t:typeof n.t=="number"?n.t:Date.now()})}typeof window<"u"&&!window.__cbsgo_playerPos_listener&&(window.__cbsgo_playerPos_listener=!0,window.addEventListener("cbsgo:playerPos",va));async function _a(){const t=Ye();if(!t||!Pt)return;const n=Date.now();if(n-mr<5e3)return;mr=n;const l=(ut()||"").trim()||"Anon",c={wallet_pk:t,nickname:l,lat:Pt.lat,lng:Pt.lng,heading:Pt.heading,last_seen:new Date().toISOString()};try{const{data:f,error:u}=await Fe.from("player_state").select("id").eq("wallet_pk",t).limit(1);if(u){console.warn("CBS GO: player_state select failed",u);return}if(f&&f.length>0){const p=f[0].id,{error:b}=await Fe.from("player_state").update(c).eq("id",p);b&&console.warn("CBS GO: player_state update failed",b)}else{const{error:p}=await Fe.from("player_state").insert(c);p&&console.warn("CBS GO: player_state insert failed",p)}}catch(f){console.warn("CBS GO: pushMyState error",f)}}async function Sa(){const t=Ye();if(!t)return;const n=Date.now();if(n-wr<3e3)return;wr=n;const o=new Date(Date.now()-wa).toISOString();try{const{data:l,error:c}=await Fe.from("player_state").select("wallet_pk, nickname, lat, lng, heading, last_seen").gt("last_seen",o);if(c){console.warn("CBS GO: fetch online players failed",c);return}const f=Array.isArray(l)?l:[],u=Array.from(new Set(f.map(x=>x.wallet_pk).filter(x=>typeof x=="string"&&x.length>0)));let p=new Map;if(u.length>0){const{data:x,error:h}=await Fe.from("players").select("wallet_pk, avatar, nickname").in("wallet_pk",u);h?console.warn("CBS GO: fetch player profiles failed",h):Array.isArray(x)&&(p=new Map(x.map(y=>[y.wallet_pk,y])))}const b=f.map(x=>{const h=x.lat,y=x.lng,_=typeof h=="number"?h:parseFloat(h),T=typeof y=="number"?y:parseFloat(y);if(!Number.isFinite(_)||!Number.isFinite(T))return null;const P=p.get(x.wallet_pk)||null,E=P&&P.nickname||x.nickname||"Anon",N=P&&P.avatar?String(P.avatar):"";return{wallet_pk:x.wallet_pk||"",nickname:E,avatar:N,lat:_,lng:T,heading:typeof x.heading=="number"?x.heading:null,last_seen:x.last_seen,isMe:x.wallet_pk===t}}).filter(Boolean);typeof window<"u"&&window.dispatchEvent(new CustomEvent("cbsgo:onlinePlayers",{detail:{players:b}}))}catch(l){console.warn("CBS GO: fetchOnlinePlayers error",l)}}function ka(){typeof window>"u"||window.__cbsgo_playerSync_started||(window.__cbsgo_playerSync_started=!0,setInterval(()=>{_a()},ha),setInterval(()=>{Sa()},ma))}ka();function Qr(){const t=Ye();if(!t)throw new Error("No local CBS-GO wallet found. Create / unlock your wallet first.");return t}function dn(t,n){console.warn(`CBS GO friends: ${t} failed`,n)}async function Ca(t){const n=Qr(),o=String(t||"").trim();if(!o)throw new Error("Wallet address is required.");if(o===n)throw new Error("You cannot add yourself as a friend.");if(o.length<20)throw new Error("That does not look like a valid wallet address.");const{error:l}=await Fe.from("friends").insert({a_wallet:n,b_wallet:o,status:"pending"});if(l)throw dn("sendFriendRequest",l),l.message&&l.message.toLowerCase().includes("duplicate")?new Error("You already have a pending or accepted friendship with this wallet."):new Error("Could not send friend request (permissions or network issue).");return{ok:!0}}async function Ea(t){const n=Qr(),o=t;if(!o)throw new Error("Invalid friend request id.");const{data:l,error:c}=await Fe.from("friends").update({status:"accepted"}).eq("id",o).eq("b_wallet",n).select("*").maybeSingle();if(c)throw dn("acceptFriendRequest",c),new Error("Could not accept friend (permissions or network issue).");if(!l)throw new Error("Friend request not found or not meant for this wallet.");return{ok:!0,friend:l}}async function Jr(){const t=Ye();if(!t)return{incoming:[],accepted:[]};const{data:n,error:o}=await Fe.from("friends").select("*").or(`a_wallet.eq.${t},b_wallet.eq.${t}`).order("created_at",{ascending:!1});if(o)throw dn("loadFriendsOverview",o),new Error("Could not load friends (permissions or network issue).");const l=Array.isArray(n)?n:[],c=[],f=[];for(const p of l){const b=p.b_wallet===t&&p.status==="pending",x=p.status==="accepted"&&(p.a_wallet===t||p.b_wallet===t);if(!b&&!x)continue;const h=p.a_wallet===t?p.b_wallet:p.a_wallet,y={id:p.id,a_wallet:p.a_wallet,b_wallet:p.b_wallet,status:p.status,created_at:p.created_at,otherWallet:h,nickname:null,avatar:""};b&&c.push(y),x&&f.push(y)}const u=Array.from(new Set([...c,...f].map(p=>p.otherWallet).filter(Boolean)));if(u.length>0){const{data:p,error:b}=await Fe.from("players").select("wallet_pk, nickname, avatar").in("wallet_pk",u);if(!b&&Array.isArray(p)){const x=new Map;for(const y of p)y.wallet_pk&&x.set(String(y.wallet_pk),{nickname:y.nickname||null,avatar:y.avatar||""});const h=y=>{y.forEach(_=>{const T=x.get(_.otherWallet);T&&(_.nickname=T.nickname||null,_.avatar=T.avatar||"")})};h(c),h(f)}else b&&dn("loadFriendsOverview:players",b)}return{incoming:c,accepted:f}}let Rt=null;async function eo(){try{"wakeLock"in navigator&&navigator.wakeLock?.request?(Rt=await navigator.wakeLock.request("screen"),console.log("CBS-GO: wake lock actief"),Rt.addEventListener("release",()=>{console.log("CBS-GO: wake lock vrijgegeven")})):console.log("CBS-GO: wake lock niet ondersteund in deze browser")}catch(t){console.warn("CBS-GO: wake lock request faalde",t)}}async function Ma(){try{Rt&&(await Rt.release(),Rt=null,console.log("CBS-GO: wake lock handmatig vrijgegeven"))}catch(t){console.warn("CBS-GO: wake lock release faalde",t)}}function La(){"wakeLock"in navigator&&document.addEventListener("visibilitychange",async()=>{try{document.visibilityState==="visible"?await eo():await Ma()}catch(t){console.warn("CBS-GO: visibility wake lock error",t)}})}const Dn="cbsgo_trades";async function Aa(t,n){const o=Ye();if(!o)throw new Error("No local CBS-GO wallet available.");const l=ut(),c=gn(),f=Number(n?.tickets||0),u=Number(n?.cbs||0),p=n?.cardId||null,b=p?Number(n?.cardQty||0):0;if(!f&&!u&&!p)throw new Error("Nothing to send.");const x=Jt(),h=en();if(f>0&&f>x)throw new Error("Not enough tickets in your bag.");if(u>0&&u>h)throw new Error("Not enough CBS (play money) in your bag.");const{error:y}=await Fe.from(Dn).insert({from_wallet:o,to_wallet:t,tickets:f,cbs:u,card_id:p,card_qty:b,sender_nickname:l||null,sender_avatar:c||null,claimed:!1});if(y)throw console.warn("CBS GO: sendGiftToWallet failed",y),new Error(y.message||"Could not send gift.");try{const _=Jt(),T=en();console.log("CBS GO: deducting from bag",{tickets:f,cbs:u,beforeTickets:_,beforeCbs:T}),f>0&&Lt(-f),u>0&&xn(-u);const P=Jt(),E=en();console.log("CBS GO: bag after deduct",{afterTickets:P,afterCbs:E}),typeof window<"u"&&(window.dispatchEvent(new CustomEvent("cbsgo:inventoryChanged")),window.dispatchEvent(new CustomEvent("cbsgo:bagChanged")))}catch(_){console.warn("CBS GO: failed to update local bag after trade",_)}}let On=!1;async function Yn(){const t=Ye();if(t&&!On){On=!0;try{const{data:n,error:o}=await Fe.from(Dn).select("*").eq("to_wallet",t).eq("claimed",!1).order("created_at",{ascending:!0}).limit(50);if(o){console.warn("CBS GO: pullIncomingGifts failed",o);return}if(!n||!n.length)return;for(const l of n){const c=l.id,{data:f,error:u}=await Fe.from(Dn).update({claimed:!0}).eq("id",c).eq("claimed",!1).select("id");if(u){console.warn("CBS GO: failed to mark trade as claimed",u);continue}if(!f||!f.length)continue;const p=Number(l.tickets||0),b=Number(l.cbs||0),x=l.card_id||null,h=Number(l.card_qty||0);p&&Lt(p),b&&xn(b),typeof window<"u"&&window.dispatchEvent(new CustomEvent("cbsgo:friendGiftReceived",{detail:{senderNickname:l.sender_nickname||"",senderAvatar:l.sender_avatar||"",toWallet:l.to_wallet,tickets:p,cbs:b,cardId:x,cardQty:h}}))}typeof window<"u"&&(window.dispatchEvent(new CustomEvent("cbsgo:inventoryChanged")),window.dispatchEvent(new CustomEvent("cbsgo:bagChanged")))}finally{On=!1}}}const to="cbsgo_solana_wallet_v1";function Ba(t,n=null){try{const o=JSON.parse(t);return!o||typeof o!="object"||typeof o.publicKey!="string"||typeof o.secretKey!="string"?n:o}catch{return n}}function za(){const t=Kr.sign.keyPair(),n=fn.encode(t.publicKey),o=fn.encode(t.secretKey),l={publicKey:n,secretKey:o,createdAt:new Date().toISOString(),version:1};try{localStorage.setItem(to,JSON.stringify(l))}catch(c){console.warn("CBS GO: failed to persist local Solana wallet",c)}return l}function Na(){try{const t=localStorage.getItem(to);return t?Ba(t,null):null}catch{return null}}function Ia(){let t=Na();return t||(t=za(),t)}function Ae(t){return String(t||"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function nr(t,n=30){const o=t?`background-image:url('${t}');`:"";return`
    <div style="
      ...
      font-size:16px;
    ">${t?"":"👤"}</div>
  `}const no="cbsgo_cards_v1";function Ta(t,n){try{const o=JSON.parse(t);return o&&typeof o=="object"?o:n}catch{return n}}function Wt(){const t=localStorage.getItem(no),n=Ta(t,{});let o={};return n&&typeof n.counts=="object"&&n.counts!==null?o={...n.counts}:Array.isArray(n.cards)&&n.cards.forEach(l=>{if(!l||!l.id)return;const c=Number(l.count||0);Number.isFinite(c)&&c>0&&(o[l.id]=c)}),o}function ro(t){const n={counts:{...t||{}}};try{localStorage.setItem(no,JSON.stringify(n))}catch{}}function qn(){const t=Wt(),n=dt();n.cards={...t||{}},Xn(n)}const $a=[{id:"walk_sun_1",label:"Sunny Walk"},{id:"walk_rain_1",label:"Rainy Walk"},{id:"walk_night_1",label:"Night Walk"},{id:"walk_city_1",label:"City Steps"},{id:"walk_nature_1",label:"Forest Trail"},{id:"walk_beach_1",label:"Beach Walk"},{id:"cbs_heart_1",label:"CBS Heart"},{id:"cbs_chain_1",label:"Break the Chain"},{id:"cbs_fire_1",label:"Builder Flame"},{id:"cbs_go_1",label:"CBS-GO Explorer"},{id:"walk_morning_1",label:"Morning Steps"},{id:"walk_evening_1",label:"Evening Glow"},{id:"walk_park_1",label:"Park Loop"},{id:"walk_bridge_1",label:"River Bridge"},{id:"cbs_star_1",label:"Community Star"},{id:"cbs_glow_1",label:"Glow Ticket"},{id:"cbs_team_1",label:"Builder Squad"},{id:"cbs_legend_1",label:"CBS Legend"},{id:"walk_placeholder_1",label:"Mystery Walk I"},{id:"walk_placeholder_2",label:"Mystery Walk II"},{id:"cbs_placeholder_1",label:"Mystery CBS I"},{id:"cbs_placeholder_2",label:"Mystery CBS II"}];function Pa(){const t=Wt();let n=0,o=0;const l=[];for(const c of $a){const f=Number(t[c.id]||0);Number.isFinite(f)&&f>0&&(n+=1,o+=f,l.push({id:c.id,count:f,label:c.label}))}return{cardTypes:n,cardTotal:o,sendable:l}}function hn(){try{return sessionStorage.getItem("cbsgo_selected_tab_v5")||"map"}catch{return"map"}}function Kn(t){try{sessionStorage.setItem("cbsgo_selected_tab_v5",t)}catch{}}function vr(t,n){return`
    <div style="
      position:fixed;
      left:0; right:0;
      bottom:0;
  ...
  `}function Oa(){const t=ut(),n=gn(),o=Ye();return`
    <section style="
      padding:14px;
      border-radius:18px;
  ...
  `}function ja(){const t=document.querySelector("#profileName"),n=document.querySelector("#profileAvatar"),o=document.querySelector("#profileRemoveAvatar");let l=null;const c=E=>{const N=document.querySelector("#profileMsg");N&&(N.textContent=E||"")};t&&c(t.value?`✅ Profile loaded: ${t.value}`:"");const f=()=>{if(!t)return;const E=Ur(t.value);c(`✅ Name saved: ${E}`);try{an()}catch(N){console.warn("CBS GO: failed to sync profile after name change",N)}};t&&(t.addEventListener("input",()=>{c("Saving…"),l&&clearTimeout(l),l=setTimeout(f,300)}),t.addEventListener("blur",()=>{l&&clearTimeout(l),f()})),n&&n.addEventListener("change",()=>{const E=n.files&&n.files[0];if(!E)return;if(E.size>15e5){c("⛔ Image too large. Please choose a smaller photo (max ~1.5MB)."),n.value="";return}c("Uploading photo…");const N=new FileReader;N.onload=()=>{si(String(N.result||"")),c("✅ Photo saved"),Mt();try{an()}catch(q){console.warn("CBS GO: failed to sync profile after avatar change",q)}},N.onerror=()=>c("⛔ Failed to read image."),N.readAsDataURL(E)}),o&&(o.onclick=()=>{li(),c("✅ Photo removed"),Mt();try{an()}catch(E){console.warn("CBS GO: failed to sync profile after avatar removal",E)}});const u=document.querySelector("#friendWalletInput"),p=document.querySelector("#friendSendBtn"),b=document.querySelector("#friendsMsg"),x=document.querySelector("#friendsIncomingList"),h=document.querySelector("#friendsAcceptedList"),y=E=>{b&&(b.textContent=E||"")},_=E=>{if(!E)return"";const N=String(E);return N.length<=12?N:`${N.slice(0,5)}…${N.slice(-4)}`},T=(E,N="")=>{const q=E.nickname&&E.nickname.trim()?E.nickname.trim():_(E.otherWallet),W=_(E.otherWallet);return`
      <div style="
        margin-bottom:6px;
        padding:6px 8px;
        border-radius:12px;
    ...
    `};async function P(){if(!(!x||!h))try{x.textContent="Loading…",h.textContent="Loading…";const E=await Jr();E.incoming.length?x.innerHTML=E.incoming.map(N=>{const q=`
            ...
            `;return T(N,q)}).join(""):h.textContent="No friends yet.",document.querySelectorAll(".friendAcceptBtn").forEach(N=>{N.addEventListener("click",async()=>{const q=N.getAttribute("data-friend-id");if(q){y("Accepting friend…"),N.disabled=!0;try{await Ea(q),y("✅ Friend added."),await P()}catch(W){console.warn(W),y(`⛔ ${W.message||W}`),N.disabled=!1}}})}),document.querySelectorAll(".friendCopyBtn").forEach(N=>{N.addEventListener("click",async()=>{const q=N.getAttribute("data-wallet")||"";if(q)try{navigator.clipboard&&navigator.clipboard.writeText?(await navigator.clipboard.writeText(q),y("✅ Friend wallet copied.")):y("📋 Copy not supported in this browser.")}catch(W){console.warn("CBS GO: copy friend wallet failed",W),y("⛔ Could not copy wallet address.")}})})}catch(E){console.warn("CBS GO: refreshFriends failed",E),x.textContent="Could not load friends.",h.textContent=""}}p&&u&&p.addEventListener("click",async()=>{const E=u.value.trim();if(!E){y("Enter a wallet address first.");return}y("Sending friend request…"),p.disabled=!0;try{await Ca(E),y("✅ Friend request sent."),u.value="",await P()}catch(N){console.warn(N),y(`⛔ ${N.message||N}`)}finally{p.disabled=!1}}),P().catch(()=>{})}function Ra(){const t=Jt(),n=en(),o=Ye(),{cardTypes:l,cardTotal:c,sendable:f}=Pa(),u=c>0?`You own ${c} cards (${l} different). You can also send some to friends as gifts.`:"You don’t have any cards yet to send.",b=f.length>0?`
