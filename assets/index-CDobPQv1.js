...
Reward: ${m.text}`;alert(v);const y={kind:r,...m};try{window.dispatchEvent(new CustomEvent("cbsgo:lootReward",{detail:y}))}catch{}}),u.addTo(P),ce=t}function bn(e){if(!h||!D||!e||V||Z<on||Math.random()>rn)return;Z=0;const t=window.L;if(!t)return;const n=et(e,60,140),o=cn(t),r=t.marker([n.lat,n.lng],{icon:o});r.on("click",()=>{if(!k){alert("GPS not ready yet. Wait until your player marker appears.");return}const s={lat:k[0],lng:k[1]},d={lat:n.lat,lng:n.lng},u=ge(s,d);if(u>Te){alert(`Too far to start this puzzle.

Distance: ${Math.round(u)}m
Needed: ≤ ${Te}m`);return}D.removeLayer(r),V=null,ae({id:`puzzle-${Date.now()}`,name:"CBS GO Puzzle"})}),r.addTo(D),V=r}function mn(){return`
